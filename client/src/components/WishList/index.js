      ...
      useEffect(()=>{
          axios.get(`http://localhost:3000/api/users/current`,{
            ...
            .then(res => {
              ...
              axios.get(`http://localhost:3000/api/wish-list/list`,{
                headers:{
                  Authorization:`Bearer ${token}`
                }
              }).then(res => {
                  ...
                  <div className= " row mt-3 border border-1 mt-4" >
                        <div className= " col">
                          ...
                          <li className="nav-item" role="presentation">
                            ...
                            <li className="nav-item" role="presentation">
                              <Link to="/wishlist" className="nav-link active" id="profile-tab"  type="button" role="tab" aria-controls="lista" aria-selected="true"><b>LISTA DE DESEOS</b></Link>
                            </li>
                            <li className="nav-item" role="presentation">
                              <Link to="/user-profile-reviews" className="nav-link" id="review"  type="button" role="tab" aria-controls="review" aria-selected="false">RESEÑAS</Link>
                            </li>
                            <li className="nav-item" role="presentation">
                              ...
                              <table className="table table-striped table-hover">
                                <tbody>
                                     {publication.length > 0 && publication.map(pub => (
                                       <Atable title={pub.title} price={pub.price} date={pub.date_publication} department={pub.depto} municipality={pub.munic} pub={pub.id_publication}></Atable>
                                     ))}      
                                </tbody>
                              </table>
                          </div>
